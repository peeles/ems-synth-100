...
describe('synth bus store', () => {
    ...
    it('removing a module clears its connections', () => {
        ...
        expect(
            bus.connections.some(c => c.from === 'mod1' || c.to === 'mod1')
        ).toBe(false)
    })
    ...
    it('exportPatch returns JSON for a patch', () => {
        const mods = [{id: 'share', type: 'osc', position: {x: 2, y: 3}}]
        bus.savePatch(mods, 'share')
        const json = bus.exportPatch('share')
        expect(JSON.parse(json)).toEqual(mods)
    })
})
